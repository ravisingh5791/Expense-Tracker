  ...
  return data.map((item) => {
    return (
      <tr key={item.categoryId}>
        ...
        <td>{item.enabled ? <span className="status-chip enabled">Enabled</span> : <span className="status-chip disabled">Disabled</span>}</td>

        <td className="table-actions">
          <button onClick={() => onEditClick(item)}>Edit</button>
          {item.enabled ? (
            <button onClick={() => disableOrEnable(item.categoryId)} style={{ background: 'linear-gradient(135deg, #e54b5d, #c83749)' }}>
              Disable
            </button>
          ) : (
            <button onClick={() => disableOrEnable(item.categoryId)} style={{ background: 'linear-gradient(135deg, #0ea872, #0a7f56)' }}>
              Enable
            </button>
          )}
        </td>
